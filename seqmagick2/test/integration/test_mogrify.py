...
class CommandLineTestMixIn(object):
    def setUp(self):
        with tempfile.NamedTemporaryFile(
            ...
            shutil.copyfile(self.input_path, self.input_file)
...
class MogrifyUngapCutTestCase(
        CommandLineTestMixIn, test_convert.ConvertUngapCutTestCase):
    command = 'mogrify --ungap --cut 1:3 --tail 2 {input}'


class MogrifyBzipInputTestCase(
        CommandLineTestMixIn, test_convert.BzipInputConvertTestCase):
    command = 'mogrify {input}'
    expected_path = data_path('output2.fasta')
    out_suffix = 'fasta.bz2'


class MogrifyGzipInputTestCase(
        CommandLineTestMixIn, test_convert.GzipInputConvertTestCase):
    command = 'mogrify {input}'
    expected_path = data_path('output2.fasta')
    out_suffix = 'fasta.gz'
